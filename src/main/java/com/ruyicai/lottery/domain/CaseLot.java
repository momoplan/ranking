...
@RooJavaBean
@RooToString
@RooEntity(versionField = "", table = "CASELOT", identifierField = "id", persistenceUnit = "lotteryPersistenceUnit")
public class CaseLot {
	...
	public static List<CaseLot> findCaseLots(int firstResult, int maxResults, String startTime, String endTime) {
		StringBuilder sql = new StringBuilder("SELECT o FROM CaseLot o WHERE o.displayState = '6' ");
		if (StringUtils.isNotBlank(startTime)) {
			sql.append(" and to_char(o.startTime,'YYYY-MM-DD') >= :startTime ");
		}
		if (StringUtils.isNotBlank(endTime)) {
			sql.append(" and to_char(o.startTime,'YYYY-MM-DD') <= :endTime ");
		}
		sql.append(" ORDER BY o.startTime DESC");
		EntityManager em = Torder.entityManager();
		TypedQuery<CaseLot> typedQuery = em.createQuery(sql.toString(), CaseLot.class).setFirstResult(firstResult)
